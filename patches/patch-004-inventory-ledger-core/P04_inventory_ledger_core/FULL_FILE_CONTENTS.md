...
./files/package.json
./files/scripts
./files/scripts/validate-patch4.mjs
./files/src
./files/src-tauri
...
./files/src-tauri/src/commands/inventory.rs
...
./files/src-tauri/src/core/inventory.rs
./files/src-tauri/src/core/migrations
./files/src-tauri/src/core/migrations.rs
./files/src-tauri/src/core/migrations/004_inventory_ledger_core.sql
...
function validatePatch3Base() {
  ...
  if (!fs.existsSync(packageJsonPath) || !fs.existsSync(patchingPath) || !fs.existsSync(migration3Path)) {
    throw new Error(
      "Patch 4 expects an existing Patch 3 project. Missing package.json, patching.rs, or migration 003."
    );
  }

  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8"));
  const patchingSource = fs.readFileSync(patchingPath, "utf8");
  const looksLikePatch3 =
    String(packageJson.version || "").startsWith("0.3") ||
    patchingSource.includes("P003_catalog_core");

  if (!looksLikePatch3 && !force) {
    throw new Error(
      "Target project does not look like a Patch 3 base. Re-run with --force only if you are sure."
    );
  }
}
...
export function DataCenterPage() {
  ...
  async function handlePreviewImport() {
    ...
    try {
      ...
      setPreviewResult(
        `Bundle OK · type=${preview.bundleType ?? "unknown"} · source=${preview.sourcePatchLevel ?? "unknown"} · generated=${preview.generatedAt ?? "unknown"} · businesses=${preview.businessCount} · categories=${preview.categoryCount} · items=${preview.itemCount} · movements=${preview.movementCount}`
      );
    } catch (error) {
      setPreviewResult(
    ...
    <div className="page-grid">
      <section className="split-grid">
        <article className="card">
          ...
          <p className="card-note">
            Patch 4 expands the export scope again so the workspace bundle now carries
            inventory movement history alongside businesses, settings, catalog items,
            units, and barcodes. Backups still stay fully local and file-based.
          </p>
      ...
      <section className="split-grid">
        <article className="card">
          ...
          <p className="card-note">
            Patch 4 still stops at preview validation, but the preview now understands
            inventory-aware workspace exports and shows business, category, item, and
            movement counts before any future import flow is attempted.
          </p>
...
version = "0.4.0"
description = "Local-first business management desktop app inventory ledger core"
authors = ["OpenAI"]
edition = "2021"
...
use crate::{
    core::{catalog, db, error::CommandResult, inventory},
...
use super::{db, error::to_command_error, inventory};
...
pub fn seed_demo_catalog_for_business(
    conn: &Connection,
    business_id: &str,
) -> rusqlite::Result<()> {
...
pub fn load_catalog_workspace(
    conn: &Connection,
    business_id: &str,
) -> Result<CatalogWorkspace, String> {
...
pub fn save_catalog_item(
    ...
    let existing_item = input
        .id
        .as_deref()
        .and_then(|item_id| get_catalog_item(conn, business_id, item_id).ok());
    let item_id = input
        .id
        .clone()
        .unwrap_or_else(|| Uuid::new_v4().to_string());
    ...
    let stock_quantity = if track_stock {
        input.stock_quantity.max(0.0)
    } else {
        0.0
    };
    let reorder_level = if track_stock {
        input.reorder_level.max(0.0)
    } else {
        0.0
    };
    let now = db::now_iso();
    ...
    let saved_item = get_catalog_item(conn, business_id, &item_id)?;
    inventory::sync_catalog_stock(conn, business_id, existing_item.as_ref(), &saved_item)?;
...
pub fn set_catalog_item_archived(
    ...
    db::insert_log(
        ...
        "INFO",
        "catalog",
        "Catalog item archive state changed",
        None,
    )?;
...
const PATCHES: [PatchDefinition; 4] = [
    PatchDefinition {
...
pub const PATCH_ID: &str = "P004_inventory_ledger_core";
pub const PATCH_NAME: &str = "Inventory Ledger Core";
...
pub fn seed_if_empty(conn: &Connection) -> rusqlite::Result<()> {
    ...
    upsert_meta(conn, "app_version", "0.4.0")?;
...
```json
{
  ...
  "description": "Adds a local inventory ledger, movement history, stock balance tracking, reorder monitoring, and inventory-aware export preview on top of Patch 3 catalog core.",
  "dependencies": [
    "P003_catalog_core"
  ],
